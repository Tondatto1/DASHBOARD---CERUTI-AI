...
export function generateSalespersonEmail(name: string): string {
  const normalized = name
    ...
    .trim()
    .replace(/\s+/g, ".");
  return `${normalized || "colaborador"}@ceruti.com.br`;
}

const INITIAL_SALESPEOPLE_WITH_EMAILS: Salesperson[] = INITIAL_SALESPEOPLE.map((s) => ({
  ...s,
  email: s.email || s.calendarIntegration?.email || generateSalespersonEmail(s.name),
}));
...
export function Dashboard({ onLogout }: DashboardProps) {
  const [salespeople, setSalespeople] = useState<Salesperson[]>(INITIAL_SALESPEOPLE_WITH_EMAILS);
  ...
  const [editingEmailPerson, setEditingEmailPerson] = useState<Salesperson | null>(null);
  ...
  const handleOpenEditEmail = (person: Salesperson) => {
    setOpenActionDropdownId(null);
    setEditingEmailPerson(person);
    setEditEmail(person.email || generateSalespersonEmail(person.name));
  };

  const handleSaveEditEmail = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingEmailPerson || !editEmail.trim()) return;
    const formattedEmail = editEmail.trim().toLowerCase();
    setSalespeople(
      salespeople.map((s) =>
        s.id === editingEmailPerson.id ? { ...s, email: formattedEmail } : s
      )
    );
    setEditingEmailPerson(null);
  };
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    const newSalesperson: Salesperson = {
      ...
      name: newName,
      email: newEmail.trim() || generateSalespersonEmail(newName),
                  ...
                  <div className="flex items-center space-x-3 w-full sm:w-auto">
                    <div className="relative w-full sm:w-72">
                      ...
                      <input
                        ...
                        placeholder="Buscar por colaborador, email ou telefone..."
                        ...
                        {filteredSalespeople.map((person, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {/* 2. EMAIL */}
                              <td className="px-5 sm:px-6 py-4 text-slate-600 font-medium text-xs whitespace-nowrap group-hover:text-emerald-700 transition-colors">
                                <button
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  className="flex items-center space-x-1.5 hover:text-emerald-700 transition-colors text-left group/email cursor-pointer"
                                  title="Editar e-mail do colaborador"
                                >
                                  <Mail className="w-3.5 h-3.5 text-slate-400 group-hover/email:text-emerald-600 transition-colors shrink-0" />
                                  <span className="font-mono text-xs text-slate-700 group-hover/email:text-emerald-700 group-hover/email:underline underline-offset-2">
                                    {person.email || generateSalespersonEmail(person.name)}
                                  </span>
                                  <Pencil className="w-3 h-3 text-slate-300 opacity-0 group-hover/email:opacity-100 group-hover/email:text-emerald-600 transition-opacity shrink-0 ml-1" />
                                </button>
                              </td>
                              ...
                              <td className="px-5 sm:px-6 py-4 text-slate-600 font-mono text-xs font-medium whitespace-nowrap group-hover:text-emerald-700 transition-colors">
                                <div className="flex items-center space-x-1.5">
                                  <Phone className="w-3 h-3 text-slate-400 group-hover:text-emerald-600 transition-colors shrink-0" />
            ...
            >
              ...
              <form onSubmit={handleAdd} className="p-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                      E-mail Corporativo <span className="text-slate-400 font-normal text-xs">(Opcional)</span>
                    </label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 hover:border-emerald-300 focus:outline-none focus:ring-2 focus:ring-[#00a83e] focus:border-transparent transition-all duration-200 text-slate-900 text-sm placeholder:text-slate-400"
                        placeholder="Ex: ana.souza@ceruti.com.br"
                      />
                    </div>
                  </div>
                </div>
        ...
        {editingEmailPerson && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              onClick={() => setEditingEmailPerson(null)}
            />
            <motion.div
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">Alterar E-mail</h3>
                    <p className="text-xs text-slate-500">{editingEmailPerson.name}</p>
                  </div>
                </div>
                <button
                  type="button"
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 hover:border-emerald-300 focus:outline-none focus:ring-2 focus:ring-[#00a83e] focus:border-transparent transition-all duration-200 text-slate-900 text-sm placeholder:text-slate-400"
                      placeholder="Ex: colaborador@ceruti.com.br"
                    />
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1.5">
                    O e-mail será atualizado na lista de acessos da equipe e filtros de busca.
                  </p>
                </div>
